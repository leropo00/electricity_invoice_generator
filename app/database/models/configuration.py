...
from sqlalchemy import  Boolean, Computed, CheckConstraint, Enum as SqlEnum, ForeignKey, Integer, String
...
class HourlyBlockLevel(Base, TimestampMixin):
    ...
    __table_args__ = (
        ...
        CheckConstraint("hour >= 0 AND hour <= 23",  name="hourly_block_levels_hours_valid"),
    )
    ...
    day_type: Mapped[SeasonDayType] = mapped_column(
        SqlEnum(SeasonDayType, name="hourly_block_levels_day_type", native_enum=True), nullable=False
    )

    electricity_season_id: Mapped[int] = mapped_column(ForeignKey("config_electricity_seasons.id"))
    electricity_season: Mapped["ElectricitySeason"] = relationship("ElectricitySeason", back_populates="hours")
